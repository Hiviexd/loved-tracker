...
async function sendBadgeWebhook({ logWebhook, pingWebhookUrl, discordPings, row, sheetRowUrl }) {
    utils.logInfo(`Sending webhook for user ${row[1]}`, logWebhook);
    ...
    try {
        ...
        utils.logError(`Failed to send webhook: ${error}`, logWebhook);
    } finally {
        // 1s timeout
...
class StatefulWebhook {
    ...
    async send(payload) {
        ...
        try {
            ...
            utils.logError(`Failed to send initial webhook message: ${error}`);
            return null;
        }
    }
    ...
    async #update(payload) {
        if (!this.#messageId) {
            utils.logError("Cannot update: no message has been sent yet. Use send() first.");
            return null;
        }
        ...
        try {
            ...
            utils.logError(`Failed to update webhook message: ${error}`);
            return null;
        }
    }
