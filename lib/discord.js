 ...
 * @param {StatefulWebhook} webhook - The stateful webhook instance (uses its webhookUrl)
 * @param {string[]} discordPings - Array of Discord user IDs to ping
 * @param {string[]} row - Row data [userId, username, newBadge, oldBadge, command]
 * @param {string} [sheetRowUrl] - Optional link to the sheet row for this user
 */
async function sendBadgeWebhook(webhook, discordPings, row, sheetRowUrl) {
    utils.consoleLog(`Sending webhook for user ${row[1]}`, webhook);
    ...
    try {
        ...
        utils.consoleError(`Failed to send webhook: ${error}`, webhook);
    } finally {
        // 1s timeout
